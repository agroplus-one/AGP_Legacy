...
<html>
	<head>
		...
		<script>
			...
			{
				/*var frm = document.getElementById('main3');
				frm.subentidad.value = esMediadora;				
				frm.oficina.value = oficina;
				frm.refpoliza.value = refpoliza;
				marcaCombo(frm.estado, estado);*/
	...
	<form:form name="main3" id="main3" action="incidencias.html" method="post" commandName="ficheroIncidenciaBean">
			<c:set var="ficheroTipo" value="${ficheroIncidenciaBean.fichero.tipofichero}"/>
		<form:hidden path="fichero.id" id="idFichero" />
		<form:hidden path="fichero.tipofichero" id="tipofichero" />
			...
			<div class="panel2 isrt" style="width: 95%;">
				...
				<table align="center">
					<tr>
						...
						<td width="100px">
							<form:input path="fichero.nombrefichero" id="fichero" size="20" maxlength="12" cssClass="dato" readonly="true" />
						</td>
						<td class="literal">Fase</td>
						<td width="80px">
							<form:input path="fichero.fase.fase" id="fase" size="5" maxlength="4" cssClass="dato" readonly="true" />
						</td>
						<td class="literal">Fec. Carga</td>
						<td width="100px">
							<spring:bind path="fichero.fechacarga">
								<input type="text" name="fichero.fechacarga" id="fecha1" size="11" class="dato" readonly="true"
									value="<fmt:formatDate pattern="dd/MM/yyyy"	value="${ficheroIncidenciaBean.fichero.fechacarga}"/>" />
							</spring:bind>
						</td>
						<td class="literal">Fec. Recibo</td>
						<td width="100px">
							<spring:bind path="fichero.fase.fechaemision">
								<input type="text" name="fichero.fase.fechaemision" id="fecha2" size="11" class="dato" readonly="true"
									value="<fmt:formatDate pattern="dd/MM/yyyy"	value="${ficheroIncidenciaBean.fichero.fase.fechaemision}"/>" />
							</spring:bind>
						</td>
						<td class="literal">Plan</td>
						<td width="80px">
							<form:input path="fichero.fase.plan" id="plan" size="5" maxlength="4" cssClass="dato" readonly="true" />
						</td>
	...
	<div id="grid">
		<display:table requestURI="" class="LISTA"
			summary="incidencias" pagesize="${numReg }" sort="list"
			name="${listIncidencias}" id="incidencias" excludedParams="method"
			decorator="com.rsi.agp.core.decorators.ModelTableDecoratorIncidencias"
			style="width:95%;border-collapse:collapse;">
		
			<display:column class="accionesDisplayTagColumn" headerClass="cblistaImg" title="Acciones" property="admActions" sortable="false" style="width:40px;text-align:left" />
			...
			<display:column class="literal" headerClass="cblistaImg" title="Colectivo" property="colectivo" sortable="true"	style="width:60px;text-align:center" />
			<c:if test="${ficheroTipo!='G'}" >
				<display:column class="literal" headerClass="cblistaImg" title="E-S Med Fich." property="esMed" sortable="true"	style="width:50px;text-align:center" />
			</c:if>
			<c:if test="${ficheroTipo!='I'}" >
				<display:column class="literal" headerClass="cblistaImg" title="Oficina" property="oficina" sortable="true" style="width:50px;text-align:center" comparator="com.rsi.agp.core.comparators.TableComparatorOficina"/>
				<display:column class="literal" headerClass="cblistaImg" title="Póliza" property="refPoliza" sortable="true" style="width:50px;text-align:center" />
			</c:if>
			<display:column class="literal" headerClass="cblistaImg" title="Estado" property="estado" sortable="true" style="width:30px;text-align:center" />
