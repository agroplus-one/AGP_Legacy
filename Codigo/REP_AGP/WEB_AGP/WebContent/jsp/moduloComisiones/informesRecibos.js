...
//DAA 01/10/2013 informesRecibos.js
...
$(document).ready(function(){
	...
	$('#main3').validate({					
		...
		rules: {
			...
			"fecha": {invalidChars: true, validaCondicionCompleta:['fecha','condiFecha'], validaIntervaloFechas:['fecha','condiFecha'], validaFecha: true},
			"recibo":{invalidChars: true, validaCondicionCompleta:['recibo','condiRecibo'], validaIntervalo:['recibo','condiRecibo'], validaLongitud:['recibo','6']},
		...
		$("input[type=checkbox]").each(function(){
			//para cada check marcado comprobamos si corresponde con el formato seleccionado
	     	if($(this).attr('checked')== true){
	     		...
	     		if(tabindex == 11 || tabindex < 10 ){
	     			//campos cumunes, se puede generar el informe	
	     			validado = true;
	     			return false;
	     		}else{
	     			if( datosDe == "resumen"  && ( tabindex == 10 || ((tabindex >= 12) && (tabindex <= 16)) )	){
     					validado = true;
     					return false;
	     			}
	     			if(datosDe == "detalle" && tabindex >= 17){
	     				validado = true;
		     			return false;
	     			}
	     		}
