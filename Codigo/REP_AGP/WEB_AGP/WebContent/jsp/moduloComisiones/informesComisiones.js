...
$(document).ready(function(){
	...
	$('#main3').validate({					
		...
		rules: {
			...
			"colectivo": {invalidChars: true, validaCondicionCompleta:['colectivo','condiColectivo'], validaIntervalo:['colectivo','condiColectivo'], validaLongitud:['colectivo','7']},
			"linea": {invalidChars: true, validaCondicionCompleta:['linea','condiLinea'], validaIntervalo:['linea','condiLinea'], validaLongitud:['linea','3']},
			"plan": {invalidChars: true, validaCondicionCompleta:['plan','condiPlan'], validaIntervalo:['plan','condiPlan'], validaLongitud:['plan','4']},
			"entidadMed": {invalidChars: true, validaCondicionCompleta:['entidadMed','condiEntidadMed'], validaIntervalo:['entidadMed','condiEntidadMed'], validaLongitud:['entidadMed','4']},
			"subent": {invalidChars: true, validaCondicionCompleta:['subent','condiSubent'], validaIntervalo:['subent','condiSubent'], validaLongitud:['subent','4']},
			"fechaAcep": {invalidChars: true, validaCondicionCompleta:['fechaAcep','condiFechaAcep'], validaIntervaloFechas:['fechaAcep','condiFechaAcep'], validaFecha: true},
			"referencia": {invalidChars: true, validaCondicionCompleta:['referencia','condiReferencia'], validaIntervalo:['referencia','condiReferencia'], validaLongitud:['referencia','7'], validaCodRef: true},
			"nif": {invalidChars: true, validaCondicionCompleta:['nif','condiNif'], validaIntervalo:['nif','condiNif'], validaLongitud:['nif','9'], nif: true},
			"idInforme": {validaChecks: true, validaResumenDetalle: true},
		...
		messages: {
			...
			"fechaAcep": {invalidChars: "El campo Fecha aceptación contiene caracteres no válidos, los valores deben ir separados por ','", validaCondicionCompleta: "Debe completar la condición para Fecha de aceptación", validaIntervaloFechas: function (value){if(document.getElementById(value[1]).value =="5")return "El campo Fecha aceptación no puede contener intervalos para la condición elegida";else return "El campo Fecha aceptación contiene un intervalo incorrecto"}, validaFecha: "La fecha introducida no es correcta"},
			"referencia": {invalidChars: "El campo Ref. Póliza contiene caracteres no válidos, los valores deben ir separados por ','", validaCondicionCompleta: "Debe completar la condición para la Ref. Póliza", validaIntervalo: function (value){if(document.getElementById(value[1]).value =="5")return "El campo Ref. Póliza no puede contener intervalos para la condición elegida";else return "El campo Ref. Póliza contiene un intervalo incorrecto"}, validaLongitud: "Cada Ref. Póliza debe contener 7 dígitos", validaCodRef: "El formato de la Ref. Póliza no es correcto"},
			"nif": {invalidChars: "El campo NIF contiene caracteres no válidos, los valores deben ir separados por ','", validaCondicionCompleta: "Debe completar la condición para el Nif", validaIntervalo: function (value){if(document.getElementById(value[1]).value =="5")return "El campo NIF no puede contener intervalos para la condición elegida";else return "El campo NIF fase contiene un intervalo incorrecto"}, validaLongitud: "Cada NIF debe contener 9 dígitos como máximo", nif: "El NIF introducido no es válido"},
			"idInforme": {validaChecks: "Debe seleccionar al menos un campo", validaResumenDetalle: "Ningún campo seleccionado corresponde con el formato elegido."},
	...
	// comprueba que los check seleccionados correspondan al tipo de formato elegido
	jQuery.validator.addMethod("validaResumenDetalle", function(value, element) {
		var validado = false;
		var datosDe = $('input:radio[name=mostrarDatos]:checked').val();
	    
		$("input[type=checkbox]").each(function(){
			//para cada check marcado comprobamos si corresponde con el formato seleccionado
	     	if($(this).attr('checked')== true){
	     		var tabindex = $(this).attr('tabindex');
	     		
	     		if ($('#esExterno').val() == '1') {  // EXTERNO Perfil 1
	     			if( ((tabindex >= 1 && tabindex <= 3) ||  (tabindex >= 6 && tabindex <= 9) || (tabindex >= 17 && tabindex <= 25) 
	     					|| (tabindex == 28) || (tabindex == 31) || (tabindex == 32) || (tabindex >= 35 && tabindex <= 37) ) ){
		     			//campos cumunes, se puede generar el informe	
		     			validado = true;
		     			return false;
		     		}else{
		     			if( datosDe == "resumen"  && ( tabindex == 10 || tabindex == 26 || tabindex == 27) )	{
	     					validado = true;
	     					return false;
		     			}
		     			if(datosDe == "detalle" && (tabindex == 4 || tabindex == 5 || ((tabindex >= 11) && (tabindex <= 16)) || 
		     					(tabindex == 29) || (tabindex == 30) || (tabindex == 33) || (tabindex == 34)) ){
		     				validado = true;
		     				return false;
		     			}
		     		}
	     			...
	     			if ( $('#perfil').val() == '1' || $('#esExterno').val() == '5' ) {
						if( ((tabindex >= 1 && tabindex <= 3) ||  (tabindex >= 6 && tabindex <= 9) || (tabindex >= 17 && tabindex <= 31) 
			     				|| (tabindex == 36) || (tabindex == 39) || (tabindex == 40) || (tabindex >= 43 && tabindex <= 45)) ){
			     			//campos cumunes, se puede generar el informe	
			     			validado = true;
			     			return false;
			     		}else{
			     			if( datosDe == "resumen"  && ( tabindex == 10 || ((tabindex >= 32) && (tabindex <= 35)) )	){
			 					validado = true;
			 					return false;
			     			}
			     			if(datosDe == "detalle" && (tabindex == 4 || tabindex == 5 || ((tabindex >= 11) && (tabindex <= 16)) || 
			     					(tabindex == 37) || (tabindex == 38) || (tabindex == 41) || (tabindex == 42)) ){
			     				validado = true;
			     				return false;
			     			}
			     		}
					}else{ //INTERNOS 0 -- OK
						if( ((tabindex >= 1 && tabindex <= 3) ||  (tabindex >= 6 && tabindex <= 9) || (tabindex >= 17 && tabindex <= 40) 
								|| (tabindex == 46) ||  (tabindex == 49) || (tabindex == 50) || (tabindex >= 53 && tabindex <= 55)  ) ){
			     			//campos cumunes, se puede generar el informe	
			     			validado = true;
			     			return false;
			     		}else{
			     			if( datosDe == "resumen"  && ( tabindex == 10 || ((tabindex >= 41) && (tabindex <= 45)) )	){
		     					validado = true;
		     					return false;
			     			}
			     			if(datosDe == "detalle" && (tabindex == 4 || tabindex == 5 || ((tabindex >= 11) && (tabindex <= 16)) || 
			     					(tabindex == 47) || (tabindex == 48) || (tabindex == 51) || (tabindex == 52)) ){
			     				validado = true;
			     				return false;
			     			}
			     		}
	...
	function validaCodRef(input){
		var validado = true;
		$('#referencia').val(input.toUpperCase());
...
function limpiar(){
	...
	$("#mostrarDatosResumen").attr('checked',true);
...
function habilitaRefYNif(){
	$('#condiReferencia').attr('disabled', false);
	$('#referencia').attr('disabled', false);
	$('#condiNif').attr('disabled', false);
	$('#nif').attr('disabled', false);
}

function deshabilitaRefYNif(){
	$('#condiReferencia').attr('disabled', true);
	$('#condiReferencia').val('');
	$('#referencia').attr('disabled', true);
	$('#referencia').val('');
	$('#condiNif').attr('disabled', true);
	$('#condiNif').val('');
