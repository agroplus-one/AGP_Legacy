...
<head>
	...
	<script language="javascript">
		...
		function volver(){
			if ($('#parcelasAnexoCplInfo').val() == 'true'){
		...
		function getModulos(idDiv,idpoliza,idTabla){
			if(!document.getElementById(idTabla)){
				modulos(idDiv,idpoliza,$('#idAnexo').val());
			}
		}
		function modulos(idDiv,idpoliza,idAnexo){
			$.ajax({
				    url: "coberturasController.html" ,
					data: "method=doDetalleModulo&idpoliza="+idpoliza+"&esAnexo=true&idAnexo="+idAnexo,
					async:true,
				    dataType: "json",
				    success: function(datos){		
				            $('#'+idDiv).html(datos.tabla);   	
				            $('#modPPal').html(datos.descripcion);
				            $('#modCpl').html(datos.descripcionCPL);
				   },
				    beforeSend: function(){
...
<body bgcolor="#FFFFFF" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" onload="SwitchMenu('sub3');">
	...
	<form name="main" action="declaracionesModificacionPolizaComplementaria.html" method="post" id="main">
		...
		<input type="hidden" name="idAnexo" id="idAnexo" value="${idAnexo}" />
		<input type="hidden" id="vieneDeListadoAnexosMod" name="vieneDeListadoAnexosMod" value="${vieneDeListadoAnexosMod}"/>
		<input type="hidden" id="parcelasAnexoCplInfo" name="parcelasAnexoCplInfo" value="${parcelasAnexoCplInfo}"/>
	</form>
	...
	<div class="conten" style="padding: 3px; width: 97%">
		...
		<fieldset  style="width:95%">
				<legend class="literal">COBERTURAS PÓLIZA PRINCIPAL&nbsp;&nbsp;&nbsp;&nbsp; Módulo <span id="modPPal">${polizaPpal.codmodulo}</span></legend>
		...
		<fieldset  style="width:95%">
				<legend class="literal">COBERTURAS PÓLIZA COMPLEMENTARIA &nbsp;&nbsp;&nbsp;&nbsp; Módulo <span id="modCpl">${polizaCpl.codmodulo}</span></legend>
