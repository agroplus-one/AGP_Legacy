...
<title>Cálculo de RC</title>
		...
		<%@ include file="/jsp/moduloUtilidades/modificacionesPoliza/utilEnviosRC.jsp"%>
</head>
...
<body bgcolor="#FFFFFF" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" onload="SwitchMenu('sub3');">
		...
		<!--<form:form name="impresionIncidenciasMod" id="impresionIncidenciasMod" action="impresionIncidenciasMod.html" method="post" commandName="reduccionCapital">
			...
			<input type="hidden" id="nombreCompleto" name="nombreCompleto" value="${reduccionCapital.poliza.asegurado.nombreCompleto}"/>
			<form:hidden path="poliza.codmodulo" id="codmodulo"/>
			<input type="hidden" id="fechaEnvio" name="fechaEnvio" value="${reduccionCapital.poliza.fechaenvio}"/>
		</form:form> -->
		
		<!-- Formulario para confirmar el anexo o volver a las pantallas desde las que se ha solicitado el envío  -->
		<form:form action="confirmacionRC.html" name="formConfirmar" id="formConfirmar"  method="post" commandName="reduccionCapital">
			<input type="hidden" name="method" id="methodContinuar" value="doConfirmarRC"/>
				...
				<tr>
					...
					<td align="right">
						...
						<c:if test="${empty modoLectura}">
							<a class="bot" href="#" onClick="javascript:confirmarAnexoRC(${errorTramite}, ${perfil34});">Enviar</a>
						</c:if>						
					</td>
				</tr>
			</table>
		...
		<div id="calculoModificacion">	
			<div class="conten" style="padding:3px;width:97%">
				...
				<%@ include file="/jsp/common/static/avisoErroresRC.jsp"%>
				...
				<c:if test="${empty requestScope.alerta}">
					...
					<jsp:include page="/jsp/moduloUtilidades/modificacionesPoliza/tablaDistribucionCostesRC.jsp">
		...
		<%@ include file="/jsp/moduloUtilidades/reduccionCapital/popupConfirmacion.jsp"%>
