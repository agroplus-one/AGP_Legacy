...
<html>
<head>
	<title>Agroplus - Detalle Reglamento</title>
		...
		<script type="text/javascript">
	        ...
	        function onInvokeAction(id) {
			    ...
			    $.get('${pageContext.request.contextPath}/ficheroReglamento.run?ajax=true&idFichero='+$("#idFichero").val() +'&' + parameterString, function(data) {
			        $("#grid").html(data)
		  			});
			}
		    ...
		    function consultar (idReglamento){
		    	var frm = document.getElementById('main2');
		    	frm.method.value="doConsulta";
		    	frm.idReglamento.value= idReglamento;
		    	$('#main2').submit();
...
<body bgcolor="#FFFFFF" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0"  onload="SwitchSubMenu('sub7', 'sub5');">
		...
		<div class="conten" style="padding:3px;width:97%" >
			<p class="titulopag" align="left">Detalle Fichero Reglamento</p>			
			
			<form name="main2" id="main2" action="ficheroReglamentoSit.run" method="post">
				<input type="hidden" name="method" id="method"/>
				<input type="hidden" name="origenLlamada" id="origenLlamada" value="detalleReglamento"/>
				<input type="hidden" name="idReglamento" id="idReglamento" />
				<input type="hidden" name="idFichero" id ="idFichero" value="${idFichero}"/>
				<input type="hidden" name="nombreFichero" id ="nombreFichero" value="${reglamentoProduccionEmitidaBean.fichero.nombrefichero}"/>
				<input type="hidden" name="estado" id ="estado" value="${estado}"/>
				<input type="hidden" name="fase" id ="fase" value="${reglamentoProduccionEmitidaBean.fichero.fase.fase}"/>
			</form>
			<form:form name="main1" id="main1" action="ficheroReglamento.run" method="post"  commandName="reglamentoProduccionEmitidaBean">
				<input type="hidden" name="method" id="method" />	
				<input type="hidden" name="idFichero" id ="idFichero" value="${idFichero}"/>
						...
						<fieldset><legend class="literal">Datos Fichero</legend>
							<table align="center">
								...
								<td class="literal">Fichero:</td>
									<td width="350px" class="detalI">${reglamentoProduccionEmitidaBean.fichero.nombrefichero}</td>
								<td class="literal">Estado:</td>
									<td width="350px" class="detalI">${estado}</td>
								<td class="literal">Fase:</td>
									<td width="350px" class="detalI">${reglamentoProduccionEmitidaBean.fichero.fase.fase}</td>
