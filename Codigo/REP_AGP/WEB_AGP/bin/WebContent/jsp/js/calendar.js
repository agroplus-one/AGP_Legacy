...
// $Id: calendar.js,v 1.1 2006/08/28 12:22:58 cvsuser01 Exp $
		...
		(function() {
			...
			cal.onSetTime = function() {
				...
				if (this.timeInterval) {
					mins += this.timeInterval - (mins % this.timeInterval);
				}
				while (mins >= 60) {
					mins -= 60;
					++hrs;
...
Zapatec.Calendar.prototype._init = function (firstDayOfWeek, date) {
	...
	if (this.getDateStatus) {
		...
		while ((status == true) || (status == "disabled")) {
			date.setTime(date.getTime() + 86400000);
			var status = this.getDateStatus(date, date.getFullYear(), date.getMonth(), date.getDate());
		}
	}
	...
	if (!this.vertical) {
		...
		minMonth = month - diffMonth;
		//alert("diffMonth = " + diffMonth + "  bottom = " + bottom + "  last_row = " + last_row + "  before_control = " + before_control);
	}
	var minYear = year;
	if (minMonth < 0) {
		minMonth += 12;
	...
	if (minYear <= this.minYear) {
		if (!this._nav_py.disabled) {
			Zapatec.Calendar._del_evs(this._nav_py);
		...
		if (this._nav_py.disabled) {	
			Zapatec.Calendar._add_evs(this._nav_py);
	...
	if (maxYear >= this.maxYear) {
		if (!this._nav_ny.disabled) {
			Zapatec.Calendar._del_evs(this._nav_ny);
		...
		if (this._nav_ny.disabled) {
			Zapatec.Calendar._add_evs(this._nav_ny);
	...
	if ((minYear == this.minYear) && (minMonth <= this.minMonth)) {
		if (!this._nav_pm.disabled) {
			Zapatec.Calendar._del_evs(this._nav_pm);
		...
		if (this._nav_pm.disabled) {	
			Zapatec.Calendar._add_evs(this._nav_pm);
	...
	if ((maxYear == this.maxYear) && (maxMonth >= this.maxMonth)) {
		if (!this._nav_nm.disabled) {
			Zapatec.Calendar._del_evs(this._nav_nm);
		...
		if (this._nav_nm.disabled) {	
			Zapatec.Calendar._add_evs(this._nav_nm);
	...
	maxDate2 = new Date(this.maxYear, this.maxMonth, 1, 0, 0, 0, 0);
	minDate1 = new Date(minYear, minMonth, 1, 0, 0, 0, 0);
	minDate2 = new Date(this.minYear, this.minMonth, date.getMonthDays(this.minMonth), 23, 59, 59, 999);
			...
			for (var k = 1; (k <= this.monthsInRow) && ((l - 1) * this.monthsInRow + k <= this.numberMonths); ++k) {
				...
				for (j = 7; cell && (iday = date.getDate()) && (j > 0); date.setDate(iday+1), cell = cell.nextSibling, --j) {
					...
					if (!cell.disabled) {
						...
						cell.ttip = "_";
						if (!this.multiple && current_month && iday == mday && this.hiliteToday && (dmonth == month)) {
