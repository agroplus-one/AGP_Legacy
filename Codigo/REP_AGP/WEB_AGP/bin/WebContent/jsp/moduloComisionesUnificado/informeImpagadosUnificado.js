...
function onInvokeAction(id) {
	...
	$.get('informeImpagadosUnificado.run?ajax=true&' +decodeURIComponent(parameterString),
			function(data) {
				$("#grid").html(data);
			});
}
...
function onInvokeExportAction(id) {
	var parameterString = $.jmesa.createParameterStringForLimit(id);
	location.href = 'informeImpagadosUnificado.run?ajax=false&'  + parameterString;
}
...
function showOrHideExportIcon() {
    var table = document.getElementById("consultaInformeImpagadosUnificado");
    var rowCount = table.rows.length;

    // Si rowCount es mayor que 1, entonces hay filas en la tabla además del encabezado.
    if (rowCount > 1) {
        // Muestra el div con el enlace y el icono de Excel.
        document.getElementById("divImprimir").style.display = "block";
    } else {
        // Oculta el div con el enlace y el icono de Excel.
