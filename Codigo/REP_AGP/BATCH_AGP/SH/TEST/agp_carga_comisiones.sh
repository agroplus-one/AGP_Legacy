...
main()
{
    ...
    log "pathJava64: ${pathJava64}"  | tee -a $LOG_NAME_1;
	...
	batchComisiones=`$pathJava64/java -jar -Xms512m -Xmx2048m -Djava.library.path=$LD_LIBRARY_PATH $fileManagerProp/CargaComisiones.jar`
	
	log "Return value: $cargaComisiones"  | tee -a $LOG_NAME_1;
	
	#log "Resultado de la ejecucion: $resultado"  | tee -a $LOG_NAME_1;
	
	if [ $batchComisiones -eq 0 ]; then
	...
	elif [ $batchComisiones -eq 3 ]; then
