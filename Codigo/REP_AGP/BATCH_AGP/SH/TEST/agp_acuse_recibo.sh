...
function doWork
{
	...
	envios=`sqlplus -s /@$oracleSIDProp << EOF
			...
			l_idpoliza NUMBER(15);
                ...
                BEGIN
                        l_query := 'SELECT IDPOLIZA, FICHERO_ENVIO FROM o02agpe0.TB_COMUNICACIONES_POLIZA WHERE TIPO_MOV=''ENVIO''';
                        OPEN l_tp_cursor FOR l_query;
                        FETCH l_tp_cursor INTO l_idpoliza, l_fichero_envio;
                        IF l_tp_cursor%FOUND THEN
                                LOOP
                                        DBMS_OUTPUT.PUT_LINE(l_idpoliza || '==' || l_fichero_envio || '##');
                                        FETCH l_tp_cursor INTO l_idpoliza, l_fichero_envio;
	...
	for envio in `echo "$envios" | grep -e "[^##]*"`; do 
		## envio viene con el formato 'poliza==fichero##'
		log "Envio: $envio"
		...
		## Después se descarga el ZIPI, donde estará el XML con el los datos del acuse de reciboP
		...
		## con el movimiento de acuse de recibo y el resultado (correcto o error)
        	updates=`sqlplus -s /@$oracleSIDProp << EOF
                ...
                set termout off;
                BEGIN
			UPDATE o02agpe0.TB_POLIZAS SET ESTADO='$estado' WHERE idpoliza=$idpoliza;
		        INSERT INTO o02agpe0.TB_COMUNICACIONES_POLIZA
			   (IDPOLIZA,
			    FECHA_ENVIO,
			    FICHERO_ENVIO,
			    TIPO_MOV,
