...
function doWork
{
	log "Inicio envio polizas sobreprecio-2";
	...
	if [ $fechas = "true" ]; then
		...
		##resultado=`$javaPath64Prop/java -jar -Xms256m -Xmx2048m -Djava.library.path=$LD_LIBRARY_PATH $fileManagerProp/SuplementosSbp.jar`
		$javaPathProp/java -jar -Djava.library.path=$LD_LIBRARY_PATH $fileManagerProp/SuplementosSbp.jar
        ...
        cierre=`sqlplus -s /@$oracleSIDProp << EOF
                ...
                begin
                        resultado := o02agpe0.pq_envio_polizas_sbp.load_polizas_sobreprecio;
		...
		if [ $ret_val_env -gt 0 ]; then
			log "Error al ejecutar el PLSQL de envio de polizas de sobreprecio. Error $ret_val_env"  | tee -a $LOG_NAME_1;				
		    log $cierre			
			exit 1;		
		fi	
...
main()
{
    ...
    LOG_NAME_1="${PATH_LOG}"/AGP_ENVIO_PLZ_SBP"${fecha}".log
