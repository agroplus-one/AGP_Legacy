#!/bin/ksh
#########################################################
## Envío alternativo de Pólizas a Agroseguro           ##
##                                                     ##
## En caso de que el servicio remoto de confirmación   ##
## no esté disponible, se utilizará el envío de        ##
## pólizas a Agroseguro por comunicaciones, de la      ##
## misma forma que se viene realizando hasta ahora,    ##
## es decir, formando un único fichero que se envía    ##
## Agroseguro. Cambia el formato de envío, según el    ##
## documento DES-N-0042.                               ##
## Tanto la selección de pólizas como la actualización ##
## de la entidad comunicaciones de póliza con el movi- ##
## miento de envío será igual a la del proceso ante-   ##
## rior. En este caso habrá que esperar a la recepción ##
## del acuse de recibo para determinar si el envío ha  ##
    ...
    do
		...
		filename=`sqlplus -s /@$oracleSIDProp << EOF
			...
			declare
				file VARCHAR2(50);
		  	begin
		  		file := o02agpe0.PQ_GENERA_ENVIOS_AGROSEGURO.genera_fichero_polizas('$codplan', '$tipopoliza');
		...
		directoryName="/aplicaciones/AGP_AGROPLUS/INTERFACES/BATCH"
		...
		log "Parametros: address=$urlUploadProp; file=$filename; folder=$directoryName; userAgro=$userAgroseguroProp; passwordAgro=$pwdAgroseguroProp"  | tee -a $LOG_NAME_1
		
		resultadoUpload=`$javaPathProp/java -jar $fileManagerProp/FileUploader.jar \
		...
		log "Resultado llamada FileUploader.java: $resultadoUpload"  | tee -a $LOG_NAME_1
