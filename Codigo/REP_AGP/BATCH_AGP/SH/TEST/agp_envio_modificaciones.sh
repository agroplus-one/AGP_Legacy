#!/bin/ksh
#########################################################
## Envío de Anexos de Modificación a Agroseguro        ##
##                                                     ##
##  Parametros: CODPLAN                                ##
##  El pl-sql genera_fichero_anexosMP requiere el      ##
...
function doWork
{
	...
	## Actualización del número de modificaciones definitivas
	resultAct=`sqlplus -s /@$oracleSIDProp << EOF
		...
		BEGIN
			--Llamada al pl
			o02agpe0.pq_genera_envios_agroseguro.actualizaModifDefinitivos;
		END;
		/
	exit
	EOF`
	...
	do
	        ...
	        log "[AA]un bucle por planes"
	
		## Se deben seleccionar los anexos de modificación del año indicado
	
		## Se llama al PL-SQL que genera los archivos para el envío de los anexos de modificación, 
		...
		filename=`sqlplus -s /@$oracleSIDProp << EOF
			...
			declare
		  	...
		  	begin
		  		fileNameToSend := o02agpe0.PQ_GENERA_ENVIOS_AGROSEGURO.genera_fichero_anexosMP('$codplan');
		...
		if [ $ret_val -gt 0 ]; then
			log "Error al ejecutar el PLSQL de generación de anexos de modificación para el Plan $codplan. Error $ret_val"  | tee -a $LOG_NAME_1;
			log "Es posible que no existan anexos de modificación para el Plan $codplan."  | tee -a $LOG_NAME_1;
			log $filename  | tee -a $LOG_NAME_1;
			
			if [ $codplan -eq $codNewPlan ]; then
				log "Continuamos con el plan anterior"
				continue
			fi
			
			if [ $ret_val -eq 150 ]; then
				#NO HAY ANEXOS PARA EL PLAN ESPECIFICADO
				exit 0
			else
				exit 1
			fi
		...
		log "Nombre del Fichero de anexo de modificacion: $filename"  | tee -a $LOG_NAME_1;
...
main()
{
    ...
    LOG_NAME_1="${PATH_LOG}"/AGP_ENVIO_MODIFICACIONES_"${fecha}".log
