#!/bin/ksh
#########################################################
## Envío de anexos de Reducción de Capital a Agroseguro##
##                                                     ##
##  Parametros: CODPLAN                                ##
##  El pl-sql genera_fichero_anexosRC requiere el      ##
...
function doWork
{
	...
	## Se deben seleccionar los anexos de reducción de capital del año indicado
	...
	## Actualización del número de reducciones de capital definitivos
	resultAct=`sqlplus -s /@$oracleSIDProp << EOF
		...
		BEGIN
			--Llamada al pl
			o02agpe0.pq_genera_envios_agroseguro.actualizaReduccDefinitivas;
		END;
		/
	exit
	EOF`
	...
	do
		## Se llama al PL-SQL que genera los archivos para el envío de los anexos de reducción de capital, 
		...
		filename=`sqlplus -s /@$oracleSIDProp << EOF
			...
			declare
		  	...
		  	begin
		  		fileNameToSend := o02agpe0.PQ_GENERA_ENVIOS_AGROSEGURO.genera_fichero_anexosRC('$codplan');
		...
		if [ $ret_val -gt 0 ]; then
			log "Error al ejecutar el PLSQL de generación de anexos de reducción de capital para el Plan $codplan. Error $ret_val"  | tee -a $LOG_NAME_1;
			log "Es posible que no existan anexos de reducción de capital para el Plan $codplan."  | tee -a $LOG_NAME_1;
		...
		log "Nombre del Fichero de reduccion de capital: $filename"  | tee -a $LOG_NAME_1;
...
main()
{
    ...
    LOG_NAME_1="${PATH_LOG}"/AGP_ENVIO_RED_CAPITAL_"${fecha}".log
