...
function doWork
{
	log "Inicio del proceso de carga de Recibos Emitidos: `date '+%d/%m/%y %H:%M:%S'`"  | tee -a $LOG_NAME_1; 	
	...
	$javaPath64Prop/java -jar -Xms512m -Xmx2048m -Djava.library.path=$LD_LIBRARY_PATH $fileManagerProp/CargaRecibosEmitidos.jar
	...
	log "Ha finalizado el proceso de carga de recibos. Se envía el correo resumen."  | tee -a $LOG_NAME_1;
	...
	resultMail=`sqlplus -s /@$oracleSIDProp << EOF
		...
		set termout off;
		BEGIN
			--Llamada al pl que genera los emails de carga de recibos y condicionados
			o02agpe0.PQ_ENVIO_CORREOS.generaCorreoResumenCargaCondRec;
		END;
		/
	exit
	EOF`
...
main()
{
    ...
    LOG_NAME_1="${PATH_LOG}"/AGP_CARGA_RECIBOS_"${fecha}".log
