#!/bin/ksh
#########################################################
## Update fecha entrada en vigor  en pólizas           ##
##                                                     ##
##                                                     ##
##                                                     ##
##                                                     ##
...
main()
{
    ...
    LOG_NAME_1="${PATH_LOG}"/UPDATE_FECHA_ENTRADA_VIGOR"${fecha}".log
	...
	$javaPath64Prop/java -jar -Xms256m -Xmx2048m -Djava.library.path=$LD_LIBRARY_PATH $fileManagerProp/UpdateFechaEntradaVigor.jar	
    
    ret_val=$?
    log "Resultado de la ejecucion: $ret_val"  | tee -a $LOG_NAME_1;
    
    if [ $ret_val -gt 0 ]; then
    	log "Error durante el update de fecha de entrda en vigor en polizas. Error $ret_val"  | tee -a $LOG_NAME_1;
    	exit 1
    fi
