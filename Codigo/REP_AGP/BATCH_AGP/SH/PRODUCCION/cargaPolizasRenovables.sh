...
main()
{
    ...
    LOG_NAME_1="${PATH_LOG}"/CARGA_POLIZAS_RENOVABLES"${fecha}".log
	...
	log "" | tee -a $LOG_NAME_1;	    	
	log "Inicio del proceso de carga de polizas renovables como renovables: `date '+%d/%m/%y %H:%M:%S'`"  | tee -a $LOG_NAME_1;    
    $javaPath64Prop/java -jar -Xms256m -Xmx2048m -Djava.library.path=$LD_LIBRARY_PATH $fileManagerProp/CargaPolizasRenovables.jar
    ...
    if [ $ret_val -gt 0 ]; then
    	log "Error durante la carga de polizas renovables como renovables"  | tee -a $LOG_NAME_1;
    	exit 1
    fi
