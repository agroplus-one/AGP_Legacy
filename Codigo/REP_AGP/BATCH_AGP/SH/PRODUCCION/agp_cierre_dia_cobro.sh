...
##                                                     ##
## Se recogerán aquellas pólizas en estado de defini-  ##
...
##                                                     ##
## En la medida de lo posible, se reutilizará el proce-##
## so de la aplicación actual para evitar discrepancias##
##                                                     ##
...
function doWork
{
	...
	## Se llama al PLSQL PQ_CIERREDIACOBRO
        cierre=`sqlplus -s /@$oracleSIDProp << EOF
                ...
                begin
                        resultado := o02agpe0.pq_cierrediacobro.load_polizas_definitivas();
	...
	log "Resultado de la ejecucion del proceso de pagos: $cierre" | tee -a $LOG_NAME_1
	
	if [ $ret_val -gt 0 ]; then
		log "Error al generar el fichero de Cierre de Dia y Cobro. Error $ret_val" | tee -a $LOG_NAME_1
		exit 1
	fi
...
main()
{
    ...
    LOG_NAME_1="${PATH_LOG}"/AGP_CIERRE_DIA_COBRO"${fecha}".log
