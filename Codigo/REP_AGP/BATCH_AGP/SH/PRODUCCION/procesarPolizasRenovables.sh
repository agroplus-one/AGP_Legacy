...
main()
{
    ...
    LOG_NAME_1="${PATH_LOG}"/PROCESAR_POLIZAS_RENOVABLES"${fecha}".log
	...
	log "" | tee -a $LOG_NAME_1;	    	
	log "Inicio del proceso de carga polizas renovables en polizas: `date '+%d/%m/%y %H:%M:%S'`"  | tee -a $LOG_NAME_1;    
    $javaPath64Prop/java -jar -Xms256m -Xmx2048m -Djava.library.path=$LD_LIBRARY_PATH $fileManagerProp/ProcesarPolizasRenovables.jar
    ...
    if [ $ret_val -gt 0 ]; then
    	log "Error durante la carga polizas renovables en polizas"  | tee -a $LOG_NAME_1;
    	exit 1
    fi
