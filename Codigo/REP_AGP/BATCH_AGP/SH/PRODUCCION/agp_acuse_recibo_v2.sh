...
function doWork
{
	...
	envios=`sqlplus -s /@$oracleSIDProp << EOF
                ...
                BEGIN
                        l_query := 'SELECT POL.IDPOLIZA, COM.IDENVIO, FICHERO_ENVIO, REFERENCIA FROM o02agpe0.TB_COMUNICACIONES COM JOIN o02agpe0.TB_POLIZAS POL ON COM.IDENVIO=POL.IDENVIO WHERE TIPO_MOV=''ENVIO'' AND RESULTADO IS NULL';
                        OPEN l_tp_cursor FOR l_query;
                        FETCH l_tp_cursor INTO l_idpoliza, l_idenvio, l_fichero_envio, l_refpoliza;
                        IF l_tp_cursor%FOUND THEN
                                LOOP
                                        DBMS_OUTPUT.PUT_LINE(l_idpoliza || '==' || l_idenvio || '==' || l_fichero_envio || '==' || l_refpoliza || '##');
                                        FETCH l_tp_cursor INTO l_idpoliza, l_idenvio, l_fichero_envio, l_refpoliza;
	...
	for envio in `echo "$envios" | grep -e "[^##]*"`; do 
		## envio viene con el formato 'poliza==idenvio==fichero==referenciapoliza##'
		log "Envio: $envio"
		...
		## con el movimiento de acuse de recibo y el resultado (correcto o error)
        	updates=`sqlplus -s /@$oracleSIDProp << EOF
                ...
                BEGIN
       			...
       			UPDATE o02agpe0.TB_COMUNICACIONES SET RESULTADO = '$resultado' WHERE IDENVIO = $idenvio AND TIPO_MOV = 'ENVIO';
		 	COMMIT;   
                END;
                /
           exit
