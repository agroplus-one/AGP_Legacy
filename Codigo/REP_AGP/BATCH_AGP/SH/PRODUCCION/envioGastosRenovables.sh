...
main()
{
    ...
    LOG_NAME_1="${PATH_LOG}"/ENVIO_GASTOS_RENOVABLES"${fecha}".log
	...
	log "" | tee -a $LOG_NAME_1;	    	
	log "Inicio del proceso de envio de gastos de polizas renovables: `date '+%d/%m/%y %H:%M:%S'`"  | tee -a $LOG_NAME_1;    
    $javaPath64Prop/java -jar -Xms256m -Xmx2048m -Djava.library.path=$LD_LIBRARY_PATH $fileManagerProp/GastosRenovables.jar
    ...
    if [ $ret_val -gt 0 ]; then
    	log "Error durante el envio de gastos de polizas renovables"  | tee -a $LOG_NAME_1;
    	exit 1
    fi
