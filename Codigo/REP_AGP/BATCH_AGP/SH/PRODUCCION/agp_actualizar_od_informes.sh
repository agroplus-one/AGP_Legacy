...
o02agpe1.pq_carga_datos_informes.actualizarOrigenesDatos;
	...
	if [ $ret_val -gt 0 ]; then
		log "Error al actualizar los datos de informes. Error $ret_val"  | tee -a $LOG_NAME_1
		log "$cierre"  | tee -a $LOG_NAME_1
		exit 1
	fi
	log $cierre
	...
	if [ $ret_val -gt 0 ]; then
		log "Error al generar el correo resumen de informes. Error $ret_val"  | tee -a $LOG_NAME_1
		log $envio  | tee -a $LOG_NAME_1
		exit 1
	fi
        log $envio
