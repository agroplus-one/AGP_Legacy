...
main()
{
	...
	log "" | tee -a $LOG_NAME_1;	    	
	log "Inicio del proceso de importacion de polizas externas: `date '+%d/%m/%y %H:%M:%S'`"  | tee -a $LOG_NAME_1;    
    $javaPath64Prop/java -jar -Xms256m -Xmx2048m -Djava.library.path=$LD_LIBRARY_PATH $fileManagerProp/ImportacionPolizasExt.jar
    ...
    if [ $ret_val -gt 0 ]; then
    	log "Error durante la importacion de polizas externas"  | tee -a $LOG_NAME_1;
    	exit 1
    fi
