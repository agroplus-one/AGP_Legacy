#!/bin/ksh
#########################################################
## Envío de cuentas de pólizas renovables a Agroseguro ##
##                                                     ##
##                                                     ##
##                                                     ##
##                                                     ##
...
##  --------- ---------- ---------- ----------------   ##
##  1.0       11/03/2016 T-SYSTEMS  1. Created.        ##
##                                                     ##
##                                                     ##
#########################################################
...
main()
{
    ...
    LOG_NAME_1="${PATH_LOG}"/ENVIO_CUENTAS_RENOVABLES"${fecha}".log
	...
	log "" | tee -a $LOG_NAME_1;	    	
	log "Inicio del proceso de envio de cuentas de renovables a Agroseguro: `date '+%d/%m/%y %H:%M:%S'`"  | tee -a $LOG_NAME_1;        
	$javaPath64Prop/java -jar -Xms256m -Xmx2048m -Djava.library.path=$LD_LIBRARY_PATH $fileManagerProp/EnvioCuentasRenovables.jar	
	...
	# Si se ha devuelto cualquier valor mayor que 0, sale con valor 1 para que la cadena finalice KO
    if [ $ret_val -gt 0 ]; then
    	log "Error durante el proceso de envio de cuentas de renovables a Agroseguro"  | tee -a $LOG_NAME_1;
    	exit 1
    fi
