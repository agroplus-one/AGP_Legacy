...
main()
{
    ...
    LOG_NAME_1="${PATH_LOG}"/AGP_BORRADO_POLIZAS_"${fecha}".log
    ...
    log "Inicio del proceso de borrado de polizas "  | tee -a $LOG_NAME_1;
    resultado=`sqlplus -s /@$oracleSIDProp << EOF
                ...
                begin
                        o02agpe0.PQ_BORRADO_POLIZAS.borradoFisico;
                end;
                /
                exit;
                EOF`   
	...
	log "Resultado del borrado de polizas: $resultado"  | tee -a $LOG_NAME_1;
	
	if [ $ret_val -gt 0 ]; then
		log "Error EN en el borrado de polizas. Error $ret_val" | tee -a $LOG_NAME_1
		exit 1
	fi
}
