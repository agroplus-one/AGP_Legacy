#!/bin/ksh
#########################################################
## Actualizar las pólizas renovables                   ##
##                                                     ##
##                                                     ##
##                                                     ##
##                                                     ##
...
##  --------- ---------- ---------- ----------------   ##
##  1.0       05/05/2015 T-SYSTEMS  1. Created.        ##
##                                                     ##
##                                                     ##
#########################################################
...
main()
{
    ...
    LOG_NAME_1="${PATH_LOG}"/UPDATE_ESTADOS_POL_RENOVABLES"${fecha}".log
	...
	log "" | tee -a $LOG_NAME_1;	    	
	log "Inicio del proceso de actualización de estados de polizas renovables: `date '+%d/%m/%y %H:%M:%S'`"  | tee -a $LOG_NAME_1;    
    $javaPath64Prop/java -jar -Xms256m -Xmx2048m -Djava.library.path=$LD_LIBRARY_PATH $fileManagerProp/UpdateEstadosPolRenovables.jar
    ...
    if [ $ret_val -gt 0 ]; then
    	log "Error durante la actualización de estados de polizas renovables"  | tee -a $LOG_NAME_1;
    	exit 1
    fi
