...
function doWork
{
	...
	resultado=`sqlplus -s /@$oracleSIDProp << EOF
                ...
                begin
                        resultadoPago := o02agpe0.PQ_RECEPCION_PAGOS_POLIZAS.recepcion_pagos_polizas_renova;	
	...
	log "Resultado de la ejecucion del proceso de recepcion de pagos de renovables: $resultado"  | tee -a $LOG_NAME_1
	
	if [ $ret_val -gt 0 ]; then
		log "Error al ejecutar el PLSQL de recepción de pagos de renovables. Error $ret_val"  | tee -a $LOG_NAME_1
		exit 1
	fi
}
...
main()
{
    ...
    LOG_NAME_1="${PATH_LOG}"/AGP_RECEPCION_PAGOS_REN_"${fecha}".log
