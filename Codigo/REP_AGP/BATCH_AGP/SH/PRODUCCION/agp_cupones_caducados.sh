...
main()
{
    ...
    LOG_NAME_1="${PATH_LOG}"/AGP_CUPONES_CADUCADOS_"${fecha}".log
    ...
    log "Inicio del proceso de actualizacion de cupones caducados "  | tee -a $LOG_NAME_1;
    resultado=`sqlplus -s /@$oracleSIDProp << EOF
                ...
                begin
                        o02agpe0.PQ_CUPON_ANEXO.revisarCuponesCaducados;			
                end;
                /
                exit;
                EOF`   
	...
	log "Resultado de la actualizacion de cupones caducados: $resultado"  | tee -a $LOG_NAME_1;
	
	if [ $ret_val -gt 0 ]; then
		log "Error EN la actualizacion de cupones caducados. Error $ret_val" | tee -a $LOG_NAME_1
		exit 1
	fi
}
