...
main()
{
    ...
    LOG_NAME_1="${PATH_LOG}"/AGP_ENVIO_FICHEROS_BD_UNIFICADA_"${fecha}".log
	...
	log "Mueve los ficheros de ejecuciones anteriores a la ruta de backup "  | tee -a $LOG_NAME_1;
	mv /aplicaciones2/AGP_AGROPLUS/INTERFACES/BATCH/*_AGRO_*.csv '/aplicaciones2/AGP_AGROPLUS/INTERFACES/BATCH/BBDD Unificada'
    ...
    log "Inicio del proceso de envio de ficheros de BD unificada "  | tee -a $LOG_NAME_1;
    resultado=`sqlplus -s /@$oracleSIDProp << EOF
                ...
                begin
                        o02agpe0.pq_genera_ficheros_bd_unif.generar_ficheros;
                end;
                /
                exit;
                EOF`   
	...
	log "Resultado del envio de ficheros de BD unificada: $resultado"  | tee -a $LOG_NAME_1;
	
	if [ $ret_val -gt 0 ]; then
		log "Error en envio de ficheros de BD unificada. Error $ret_val" | tee -a $LOG_NAME_1
		exit 1
	fi
	
	log "Fin del proceso "  | tee -a $LOG_NAME_1;
}
