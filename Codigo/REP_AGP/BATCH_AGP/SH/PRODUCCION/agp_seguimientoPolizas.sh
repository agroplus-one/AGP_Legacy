...
function doWork
{
	...
	echo "Comenzamos con el proceso de seguimiento de pólizas."        

	$javaPath64Prop/java -jar -Xms512m -Xmx2048m -Djava.library.path=$LD_LIBRARY_PATH $fileManagerProp/SeguimientoPolizas.jar
    ...
    if [ $resultado -gt 0 ]; then
    	log "Error durante el proceso de seguimiento de pólizas. Error $resultado"  | tee -a $LOG_NAME_1;
    	exit 1
    fi
	
	log "Resultado del seguimiento: $resultado"  | tee -a $LOG_NAME_1;

	log "Ha finalizado el proceso de seguimiento de polizas. Se envía el correo resumen."  | tee -a $LOG_NAME_1;
	
	## Envio del correo resumen con los resultados del seguimiento de polizas
	resultMail=`sqlplus -s /@$oracleSIDProp << EOF
		...
		set termout off;
		BEGIN
			--Llamada al pl que genera los emails de seguimiento de polizas
			o02agpe0.PQ_SEGUIMIENTO_POLIZAS.generaCorreoResumen;
			o02agpe0.PQ_SEGUIMIENTO_POLIZAS.generaCorreoTomadores;
		END;
		/
	exit
	EOF`
...
main()
{
    ...
    LOG_NAME_1="${PATH_LOG}"/AGP_SEGUIMIENTO_PLZ_"${fecha}".log
