...
public class CalculoModificacionController extends BaseMultiActionController {
	
	private ICalculoModificacionManager calculoModificacionManager;
	 ...
	 * @return
	 */
	public ModelAndView doCalculoModificacion (HttpServletRequest request, 	HttpServletResponse response, AnexoModificacion anexoModificacion) {
		...
		logger.debug("CALCULO MODIFICACION: ");
		
		for (Cobertura c: anexoModificacion.getCoberturas()) {
		...
		if (dctoPctComisiones!=null && dctoPctComisiones.length>0) {
			try {
				calculoModificacionManager.actualizaAnexoDescuento(dctoPctComisiones, anexoModificacion.getId());
				actualizaComMediadora = true;
			} catch (Exception e) {
				logger.error("Error al actualizar el descuento elegido del anexo",e);
			}				
		}
		...
		if (recargoPctComisiones!=null && recargoPctComisiones.length>0) {
			try {
				calculoModificacionManager.actualizaAnexoRecargo(recargoPctComisiones, anexoModificacion.getId());
				actualizaComMediadora = true;
			} catch (Exception e) {				
				logger.error("Error al actualizar el recargo elegido del anexo",e);
			}
		}
		AnexoModificacion anexo = null;
		try {	
			anexo = calculoModificacionManager.getAnexo(new Long(anexoModificacion.getId()));
			logger.info("ANEXO NUEVO:");
			if (anexo != null) {
				for (Cobertura c: anexo.getCoberturas()) {
					logger.info(c.getCodconcepto() + " - " + c.getCodvalor());
				}
			}
		...
		Map<String, Object> resultado = calculoModificacionManager.calcularModificacion(realPath, anexoModificacion.getId(), usuario,actualizaComMediadora);
		...
		try {
			...
			logger.error("Error en la lógica de los botones de Descuentos y recargos del anexo",e);
		}
		
		resultado.put("hayCambiosDatosAsegurado", request.getParameter("hayCambiosDatosAsegurado"));
