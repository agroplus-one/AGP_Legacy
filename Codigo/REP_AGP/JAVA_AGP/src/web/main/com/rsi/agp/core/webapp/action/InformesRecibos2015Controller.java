...
/**
 * Versi�n para los recibos de 2015 o superiores
 * @author U029823
 *
 */
public class InformesRecibos2015Controller extends BaseMultiActionController{
	...
	private Log logger = LogFactory.getLog(InformesRecibos2015Controller.class);
	 ...
	 * @return
	 */
	public ModelAndView doConsulta(HttpServletRequest request, HttpServletResponse response, InformeRecibos2015 informeRecibosBean) {
		...
		mv = new ModelAndView("moduloComisiones/informesRecibos2015", "informeRecibosBean", informeRecibosBean).addAllObjects(parameters);
		return mv;
	 ...
	 * @throws Exception
	 */
	public ModelAndView doGenerar(HttpServletRequest request,HttpServletResponse response, InformeRecibos2015 informeRecibosBean) {
		//ESC-32388
		logger.error("[FAKE ERROR] Previo a try y propiedades iniciales");
		logger.debug("init - doGenerar");
		ModelAndView mv = null;
		Map<String, Object> parameters = new HashMap<String, Object>();
		Usuario usuario = (Usuario) request.getSession().getAttribute("usuario");
		String formatoInforme = informeRecibosBean.getFormato();
		Long idCampoOculto = new Long(bundle.getString("informes.recibos2015.idInforme"));
		Informe informe = new Informe();
		informe.setOculto(new BigDecimal (idCampoOculto));
		Float numRows = null;
		try {			
			Map<String, Object> map = generacionInformeService.generarInforme2015(informe, usuario, null , informeRecibosBean);
			...
			logger.error("[FAKE ERROR] Previo a JRProperties");
			JRProperties.setProperty("net.sf.jasperreports.awt.ignore.missing.font", "true");
			DynamicReport dr = tablaInformeGen.buildReport(tabla,map);
			logger.error("[FAKE ERROR] Previo a JRDataSource");
			JRDataSource ds = null;
			logger.error("[FAKE ERROR] Previo a JasperPrint");
			JasperPrint jp = new JasperPrint();
			logger.error("[FAKE ERROR] Previo a JRTableModelDataSource");
	        ...
	        switch (Integer.parseInt(formatoInforme)) {
	    		...
	    		case ConstantsInf.COD_FORMATO_PDF:
					...
					logger.error("[FAKE ERROR] Previo a response.getOutputStream()");
					out = response.getOutputStream();
					logger.error("[FAKE ERROR] Previo a pdfExporter.setParameter()");
					...
					logger.error("[FAKE ERROR] Previo a pdfExporter.exportReport()");
	 ...
	 * @throws Exception
	 */
	public ModelAndView doOrdenar(HttpServletRequest request,HttpServletResponse response, InformeRecibos2015 informeRecibosBean) {
		...
		try {
			Map<String, Object> map = mtoInformeService.generarConsultaInforme2015(informeHTML, usuario, null, informeRecibosBean);
	...
	public void doDetalle(HttpServletRequest request,HttpServletResponse response) {
		...
		Informe informeHTML = (Informe) request.getSession().getAttribute("informeHTML");
		InformeRecibos2015 informeRecibosBean = (InformeRecibos2015) request.getSession().getAttribute("informeRecibosBean");
		
		try {
			informeRecibosBean = mtoInformeService.setInformeRecibosBeanDetalle2015(informeRecibosBean, stringRegistro);
			Map<String, Object> map = generacionInformeService.generarInforme2015(informeHTML, usuario, null , informeRecibosBean);
