package com.rsi.agp.dao.tables.reduccionCap;
// Generated by T-Systems


import java.math.BigDecimal;
import java.util.Date;

public class HistoricoEstado  implements java.io.Serializable {


     private BigDecimal id;
     private ReduccionCapital reduccionCapital;
     private String codusuario;
     private Date fecha;
     private BigDecimal estado;

    public HistoricoEstado() {
    }

	
    public HistoricoEstado(BigDecimal id, ReduccionCapital reduccionCapital, Date fecha, BigDecimal estado) {
        this.id = id;
        this.reduccionCapital = reduccionCapital;
        this.fecha = fecha;
        this.estado = estado;
    }
    public HistoricoEstado(BigDecimal id, ReduccionCapital reduccionCapital, String codusuario, Date fecha, BigDecimal estado) {
       this.id = id;
       this.reduccionCapital = reduccionCapital;
       this.codusuario = codusuario;
       this.fecha = fecha;
       this.estado = estado;
    }
   
    public BigDecimal getId() {
        return this.id;
    }
    
    public void setId(BigDecimal id) {
        this.id = id;
    }
    public ReduccionCapital getReduccionCapital() {
        return this.reduccionCapital;
    }
    
    public void setReduccionCapital(ReduccionCapital reduccionCapital) {
        this.reduccionCapital = reduccionCapital;
    }
    public String getCodusuario() {
        return this.codusuario;
    }
    
    public void setCodusuario(String codusuario) {
        this.codusuario = codusuario;
    }
    public Date getFecha() {
        return this.fecha;
    }
    
    public void setFecha(Date fecha) {
        this.fecha = fecha;
    }
    public BigDecimal getEstado() {
        return this.estado;
    }
    
    public void setEstado(BigDecimal estado) {
        this.estado = estado;
    }




}


