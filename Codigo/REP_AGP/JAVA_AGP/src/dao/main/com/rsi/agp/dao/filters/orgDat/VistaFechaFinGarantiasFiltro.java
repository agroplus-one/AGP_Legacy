...
public class VistaFechaFinGarantiasFiltro implements Filter 
{
	...
	public VistaFechaFinGarantiasFiltro() 
	{
		
	}

	public VistaFechaFinGarantiasFiltro(String codmodulo, BigDecimal codcultivo, BigDecimal codvariedad, Long lineaSeguroId, BigDecimal codprovincia, BigDecimal codcomarca, BigDecimal codtermino, Character  subtermino, BigDecimal codtipocapital, BigDecimal codpracticacultural) 
	{
	...
	private final Conjunction getFinGarantias() 
	{
		...
		{
			c.add(Restrictions.eq("variedad.id.codcultivo", this.codcultivo));
		}
		
		if (this.codvariedad != null) 
		{
			c.add(Restrictions.eq("variedad.id.codvariedad", this.codvariedad));
		}
		
		if (this.codprovincia != null){
			c.add(Restrictions.eq("codprovincia", this.codprovincia));
		}
		
		if (this.codcomarca != null){
			c.add(Restrictions.eq("codcomarca", this.codcomarca));
		}
		
		if (this.codtermino != null){
			c.add(Restrictions.eq("codtermino", this.codtermino));
		}
		
		if (this.subtermino != null){
			c.add(Restrictions.eq("subtermino", this.subtermino));
		}
		...
		if (this.codpracticacultural != null){
			// Se filtra por el valor introducido para la práctica cultural y para nulo, por si no aplica
			c.add(Restrictions.or(Restrictions.eq("practicaCultural.codpracticacultural", this.codpracticacultural), Restrictions.isNull("practicaCultural.codpracticacultural")));
		}
