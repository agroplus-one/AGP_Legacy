...
public class VistaSistemaCultivoFiltro implements Filter{
    ...
    private BigDecimal  codsistcultivo;
    ...
    private List<BigDecimal> listSistemaCultivos;
    
    
	public VistaSistemaCultivoFiltro() {		
	}

	public VistaSistemaCultivoFiltro(Long lineaSeguroId, String codmodulo,
	...
	public void setListas(List<BigDecimal> listProvincias, List<BigDecimal> listComarcas, List<BigDecimal> listTerminos, List<BigDecimal> listCultivos, List<BigDecimal> listVariedades, List<Character> listSubterminos, List<BigDecimal> listSistemaCultivos){
		...
		this.listSistemaCultivos = listSistemaCultivos;
	}
	...
	public Criteria getCriteria(Session sesion) {
		Criteria criteria = sesion.createCriteria(VistaSistemaCultivo.class);
		criteria.add(getSistemaCultivo());
		
		return criteria;
	}
	
	private final Conjunction getSistemaCultivo() {
		...
		if(FiltroUtils.noEstaVacio(this.codcultivo)){
			...
			if(listCultivos != null && listCultivos.size() > 0){
		...
		//COMENTADO POR ASF: LA VARIEDAD SI PUEDE SER '0' Y EL METODO DE FILTROUTILS NO LO ACEPTA!!!
		...
		if(!StringUtils.nullToString(this.codvariedad).equals("")){
			...
			if(listVariedades != null && listVariedades.size() > 0){
				if(!listVariedades.contains(new BigDecimal("999"))){
		...
		if(FiltroUtils.noEstaVacio(this.codprovincia)){
			...
			if(listProvincias != null && listProvincias.size() > 0){
				if(!listProvincias.contains(new BigDecimal("99"))){
		...
		if(FiltroUtils.noEstaVacio(this.codcomarca)){
			...
			if(listComarcas != null && listComarcas.size() > 0){
				if(!listComarcas.contains(new BigDecimal("99"))){
		...
		if(FiltroUtils.noEstaVacio(this.codtermino)){
			...
			if(listTerminos != null && listTerminos.size() > 0){
		...
		if(FiltroUtils.noEstaVacio(this.subtermino)){
			...
			if(listSubterminos != null && listSubterminos.size() > 0){
				if(!listSubterminos.contains(new Character('9'))){
		...
		//codsistemacultivos
		if(listSistemaCultivos != null && listSistemaCultivos.size() > 0){
	...
	public BigDecimal getCodsistcultivo() {
		return codsistcultivo;
	}

	public void setCodsistcultivo(BigDecimal codsistcultivo) {
		this.codsistcultivo = codsistcultivo;
	}

	public String getDessistemacultivo() {
		return dessistemacultivo;
	}
