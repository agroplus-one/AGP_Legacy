...
public class FicheroEmitidosService implements IFicheroEmitidosService{
	...
	public static final String CAMPO_APELLIDO2       ="apellido2";
	 ...
	 public int getFicheroEmitidosCountWithFilter(EmitidosFilter filter,
			 ReciboEmitido emitido) {
		 return emitidosDao.getFicheroEmitidosCountWithFilter(filter,
				 emitido);
	}
	
	public Collection<ReciboEmitido> getFicheroEmitidosWithFilterAndSort(
			EmitidosFilter filter, EmitidosSort sort, int rowStart,
			int rowEnd,  ReciboEmitido emitido) throws BusinessException {

		return emitidosDao.getFicheroEmitidosWithFilterAndSort(filter, sort,
				rowStart, rowEnd, emitido);
	}
	
	
	@Override
	public String getTablaEmitidos(HttpServletRequest request,
		...
		TableFacade tableFacade = crearTableFacade(request, response,
				emitido, origenLlamada);

		setDataAndLimitVariables(tableFacade, emitido);
	...
	private void configurarColumnas(HtmlTable table) {
		...
		configColumna(table, CAMPO_RECIBO, "Recibo", true, true,
				"5%");
		configColumna(table, CAMPO_CODIGO, "C&oacutedigo", false, false,
				"5%");
		configColumna(table, CAMPO_DEUDOR, "Deudor", false, false,
				"4%");
	...
	private void cargarColumnas(TableFacade tableFacade) {
		...
		tableFacade.setColumnProperties(CAMPO_ID,CAMPO_LINEA,CAMPO_RECIBO, CAMPO_CODIGO,CAMPO_DEUDOR,
				CAMPO_RAZONSOCIAL
				);

	}
	private void cargarFiltrosBusqueda(ReciboEmitido emitido,
			TableFacade tableFacade) {
		if (FiltroUtils.noEstaVacio(emitido.getFichero().getId()))
			tableFacade.getLimit().getFilterSet().addFilter(
					"fichero.id",
					emitido.getFichero().getId().toString());
	...
	private void setDataAndLimitVariables(TableFacade tableFacade,
		...
		Limit limit = tableFacade.getLimit();
		EmitidosFilter emitidosFilter = getEmitidosFilter(limit);

		// Obtiene el número de filas que cumplen el filtro
		int totalRows = getFicheroEmitidosCountWithFilter(emitidosFilter,
				emitido);
		logger
		...
		EmitidosSort emitidosSort = getEmitidosSort(limit);
		int rowStart = limit.getRowSelect().getRowStart();
		int rowEnd = limit.getRowSelect().getRowEnd();

		// Obtiene los registros que cumplen el filtro
		Collection<ReciboEmitido> items = new ArrayList<ReciboEmitido>();
		
		try {
			items = getFicheroEmitidosWithFilterAndSort(emitidosFilter,
					emitidosSort, rowStart, rowEnd, emitido);
			logger.debug("********** Items de la lista   = "
					+ items.size() + " **********");
			for (Iterator<ReciboEmitido> iterator = items.iterator(); iterator.hasNext();) {
				ReciboEmitido c = (ReciboEmitido) iterator.next();
	 ...
	 * @return
	 */
	private EmitidosFilter getEmitidosFilter(Limit limit) {
		EmitidosFilter emitidosFilter = new EmitidosFilter();
		...
		for (Filter filter : filters) {
			...
			emitidosFilter.addFilter(property, value);
		}
		return emitidosFilter;
	}
	 ...
	 * @return
	 */
	private EmitidosSort getEmitidosSort(Limit limit) {
		EmitidosSort emitidosSort = new EmitidosSort();
		SortSet sortSet = limit.getSortSet();
		Collection<Sort> sorts = sortSet.getSorts();
		for (Sort sort : sorts) {
			String property = sort.getProperty();
			String order = sort.getOrder().toParam();
			emitidosSort.addSort(property, order);
		}
