...
public class TipoSubvencionENESAXMLParser extends GenericXMLParser {
	
	public static void main(String[] args){
		
		//TEMPORAL
		/*args = new String[4];
		args[0] = "D:\\borrar\\TiposSubvencionesENESA.xml";
		args[1] = "D:\\borrar\\TiposSubvencionesENESA.csv";
		args[2] = "null";
		args[3] = "null";*/
		//FIN TEMPORAL
		
		if (args.length != 4) {
			System.out.println("Usage: java " + TipoSubvencionENESAXMLParser.class.getName()
					+ " <XML that needs to be transformed>" + " <Output file>" + " <LineaseguroId>" + " <dateFormat>");
			System.exit(1);
		}
		try {
			TipoSubvencionENESAXMLParser parser = new TipoSubvencionENESAXMLParser();
			parser.setTagPrincipal(GenericXMLParser.TAG_RG);
			parser.procesarFichero(args[0], args[1], new Long(args[2]), args[3]);
		} catch (FileNotFoundException e) {
			System.out.println("Error al buscar el fichero el fichero de Tipos Subvenciones ENESA " + e.getMessage());
			System.exit(2);
		} catch (SAXException e) {
			System.out.println("Error al crear el XMLReader " + e.getMessage());
			System.exit(3);
		} catch (IOException e) {
			System.out.println("Error de entrada/salida al parsear el fichero de Tipos Subvenciones ENESA " + e.getMessage());
			System.exit(4);
	...
	protected String generaInsert(Object reg, String dateFormat) {
		TipoSubvencionEnesa registro = (TipoSubvencionEnesa)reg;
		String sql = "";
		sql += registro.getCodtiposubvenesa() + ";" + StringUtils.nullToString(registro.getDestiposubvenesa()) + ";";
	...
	protected Object generaRegistro(Object actual, String tag, XMLStreamReader parser, int id, Long lineaseguroid) {
		TipoSubvencionEnesa registro;
		if (actual == null){
			registro = new TipoSubvencionEnesa();
		}
		else{
			registro = (TipoSubvencionEnesa) actual;
		}
		
		if (this.getTagPrincipal().equals(tag)){
			if (!StringUtils.nullToString(parser.getAttributeValue(null, "tipSubv")).equals("")){
				registro.setCodtiposubvenesa(new BigDecimal(parser.getAttributeValue(null, "tipSubv")));
			}
			
			if (!StringUtils.nullToString(parser.getAttributeValue(null, "desc")).equals("")){
				registro.setDestiposubvenesa(parser.getAttributeValue(null, "desc").trim());
			}
