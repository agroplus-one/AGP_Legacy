...
import es.agroseguro.serviciosweb.impresionscpoliza.ObtenerPolizaOriginalPDFRequest;
import es.agroseguro.serviciosweb.impresionscpoliza.ObtenerPolizaOriginalPDFResponse;
 ...
 *
 */
public class ServicioPolizaOrigenPDFHelper {private static final Log logger = LogFactory.getLog(ServicioPolizaActualPDFHelper.class);

	/**
	 * Configura y realiza la llamada al web service de pdf actual de la poliza
	 * @throws Exception 
	 */
	public Base64Binary doWork(Poliza poliza,String realPath) throws Exception{		
	...
	//	Montamos el XML de envio con los parametros
		ObtenerPolizaOriginalPDFRequest parameters = new ObtenerPolizaOriginalPDFRequest();	
		
		parameters.setCodplan(new Integer(poliza.getLinea().getCodplan().toString()));		
		parameters.setReferencia(poliza.getReferencia());
		parameters.setTiporeferencia(PolizaReferenciaTipo.fromValue(poliza.getTipoReferencia().toString()));
		
		
		
	//	Respuesta del webService	
		ObtenerPolizaOriginalPDFResponse response = null;
	
		try {		
			response = srvImpresion.obtenerPolizaOriginalPDF(parameters);
			if(response!=null){
				return response.getDocumento();		
			}
		} catch (AgrException e) {		
			WSUtils.debugAgrException(e);
			throw new Exception("Error al llamar al web service de impresión",e);
		}						
		return null;
	}
}
