...
import com.rsi.agp.dao.tables.cpl.MascaraFechaContrataAgricolaId;

public class MascaraFechasContratacionAgricolaXMLParser extends GenericXMLParser {
	
	public static void main(String[] args){
		
		//TEMPORAL
		/*args = new String[3];
		args[0] = "D:\\borrar\\MascaraFechasContratacionAgricola.xml";
		args[1] = "D:\\borrar\\MascaraFechasContratacionAgricola.csv";
		args[2] = "181";
		args[3] = "null";*/
		//FIN TEMPORAL
		...
		if (args.length != 4) {
			System.out.println("Usage: java "
					+ MascaraFechasContratacionAgricolaXMLParser.class.getName()
					+ " <XML that needs to be transformed>" + " <Output file>" + " <LineaseguroId>" + " <dateFormat>");
			System.exit(1);
		}
		try {
			MascaraFechasContratacionAgricolaXMLParser parser = new MascaraFechasContratacionAgricolaXMLParser();
			parser.setTagPrincipal(GenericXMLParser.TAG_RG);
			parser.procesarFichero(args[0], args[1], new Long(args[2]), args[3]);
		} catch (FileNotFoundException e) {
			System.out.println("Error al buscar el fichero el fichero de máscara de fechas de contratación agricola " + e.getMessage());
			e.printStackTrace();
			System.exit(2);
			...
			System.out.println("Error de entrada/salida al parsear el fichero de máscara de fechas de contratacion agricola " + e.getMessage());
			e.printStackTrace();
			System.exit(4);
	...
	protected String generaInsert(Object reg, String dateFormat) {
		MascaraFechaContrataAgricola registro = (MascaraFechaContrataAgricola)reg;
		String sql = "";
	...
	protected Object generaRegistro(Object actual, String tag, XMLStreamReader parser, int id, Long lineaseguroid) {
		MascaraFechaContrataAgricola registro;
		if (actual == null){
			registro = new MascaraFechaContrataAgricola();
		}
		else{
			registro = (MascaraFechaContrataAgricola) actual;
		}
		
		if (this.getTagPrincipal().equals(tag)){
			MascaraFechaContrataAgricolaId idMasc = new MascaraFechaContrataAgricolaId();
