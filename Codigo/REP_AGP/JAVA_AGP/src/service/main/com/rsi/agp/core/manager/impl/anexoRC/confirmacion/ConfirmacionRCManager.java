...
public class ConfirmacionRCManager implements IConfirmacionRCManager {
	...
	private static final String RC_REF = "La reduccion de capital ";
	
	@Override
	public CuponRC getCuponRCByIdCupon(Long idCuponRC) {
		
		try {
			CuponRC cuponRC = reduccionCapitalDao.getCuponRCByIdCuponRC(idCuponRC);
			return cuponRC;
	...
	public ReduccionCapital getRCByIdRC(Long id) {
		try {
			ReduccionCapital reduccionCapital = reduccionCapitalDao.getRCByIdRC(id);
			return reduccionCapital;
		} catch (DAOException e) {
			logger.error("Ha ocurrido un error al obtener la RC con id "
					+ id, e);
		}
		return null;
	}
	...
	@Override
	public AcuseRecibo generarPolizaActualizada(ReduccionCapital reduccionCapital, String realPath, String codUsuario,
			boolean hayCambiosDatosAsegurado) {
		AcuseRecibo validarPplSituacionAct = null;
		// Obtiene el objeto Poliza correspondiente al xml de la situacion actualizada
		//XmlObject p = solicitudReduccionCapManager.getPolizaActualizadaFromCupon(reduccionCapital.getCupon().getIdcupon());
		String xmlString="";
		XmlObject p = null;
		try {
			xmlString = clobToString(reduccionCapital.getXml());
			p = XmlObject.Factory.parse(xmlString);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		// Se actualiza el el objeto de ReduccionCapital con el xml de la poliza
		// actualizada
		//actualizarXMLRedCap(p, reduccionCapital);
		try {
			validarPplSituacionAct = webServicesPplManager.validarSituacionActRC(p, reduccionCapital, realPath);
			
		}catch(BusinessException e1) {
			logger.error("Error al validar la poliza", e1);			
		}finally {
			if (p != null)
			registrarComunicacionSW(codUsuario, reduccionCapital,xmlString,
	...
	@Override
	public Map<String, String> confirmarReduccionCapital(ReduccionCapital reduccionCapital, boolean isRevisionAdmin,
			String codUsuario, String realPath) {

		AcuseRecibo acRedCapital = null;
		String errorSW = null;
		Map<String, String> mensajes = new HashMap<String,String>();
		try {
			acRedCapital = swAnexoRCHelper.envioModificacionRC(reduccionCapital.getCupon().getIdcupon(), false, reduccionCapital.getXml(), null, realPath);
			
			mensajes = generarMsgConfirmacion(isRevisionAdmin, reduccionCapital, acRedCapital);
			
			if(acRedCapital!=null) {
				//datos para settear tras confirmacion
				Date fechaActual = Calendar.getInstance().getTime();
				
				reduccionCapital.setFechaenvio(fechaActual);
				
				//saber con que comparar para emplear uno de estos tres codigos que aparecen en AM
				//A	Aceptada
				//E	En revisión Admin.
				//P	Aceptada Parcialmente
				//R	Rechazada
				EstadosInc estadoAgroRC = new EstadosInc();
				estadoAgroRC.setCodestado(null);
				
				if (acRedCapital.getDocumentoArray() != null) {
					Documento documento = acRedCapital.getDocumentoArray(0);
					
					if(documento != null) {
						DatosAsociados datoAsociado = documento.getDatosAsociados();
						
						if(datoAsociado != null) {
							Node datoAsociadoEstado = datoAsociado.getDomNode();
							
							if(datoAsociadoEstado != null && datoAsociadoEstado instanceof Element) {
								Element datosAsociadosElement = (Element) datoAsociadoEstado;
								
								String estado = datosAsociadosElement.getAttribute(Constants.STR_ESTADO_KEY);
								estadoAgroRC.setCodestado(estado.charAt(0));
								
								logger.debug("El nodo indicado tiene valor de estado: " + estado );
							}else {
								logger.debug("El nodo indicado no tiene un estado valido por lo que se enviara null a BBDD");
							}
						}
					}
				}
				
				//en principio estado es correcto segun parametricas
				reduccionCapital.setEstadoAgroseguro(estadoAgroRC);
				
				//ver que valor se pasa en idestado
				//datos para settear tras confirmacion
				
				reduccionCapitalDao.actualizar(reduccionCapital);
				
				//debuge nullpointer
				BigDecimal idEstadoRC = new BigDecimal(Short.toString(reduccionCapital.getEstado().getIdestado()));
				historicoEstadosManager.insertaEstado(Tabla.ANEXO_RED, reduccionCapital.getId(), codUsuario,
						idEstadoRC);

				//falta ver si pasa adelante el intercambio y setea bien la tercera tabla
				insertarEnviosSWConfirmacion(reduccionCapital, isRevisionAdmin, codUsuario, reduccionCapital.getXml(),
						acRedCapital != null ? (acRedCapital.toString()) : errorSW);
			}
			
		} catch (AgrException e) {
			logger.error("El SW ha devuelto un error, la reduccion de capital no ha sido confirmada.", e);
			errorSW = getMsgError(e, reduccionCapital);
			mensajes.put("alerta", errorSW);
		} catch (DAOException e) {
			logger.error("Error al actualizar el rc", e);
		} catch (Exception e) {
			logger.error("Error al confirmar la reduccion de capital de poliza con AGRO", e);	
		}
		
		return mensajes;
	}
	
	private void insertarEnviosSWConfirmacion(ReduccionCapital rc, boolean indRevAdm, String codusuario,
			Clob xmlPoliza, String xmlAcuse) {

		EnviosSWConfirmacionRC envio = new EnviosSWConfirmacionRC();
		envio.setCodusuario(codusuario);
		envio.setFecha(new Date());
		envio.setIdcupon(new BigDecimal(rc.getCupon().getId()));
		envio.setIndRevAdm(indRevAdm ? 'S' : 'N');			  
		envio.setReduccionCapital(rc);

		EnviosSWXMLRC objXmlPoliza = new EnviosSWXMLRC();
		...
		envio.setEnviosSWXMLRCByIdxmlPoliza(objXmlPoliza);
		envio.setEnviosSWXMLRCByIdxmlAcuse(objXmlAcuse);

		try {
			enviosSWConfirmacionDao.guardarConfirmacionRC(envio);
		} catch (DAOException e) {
	...
	private String getMsgError(AgrException e, ReduccionCapital reduccionCapital) {

		String errorSW = getStrProperties("mensaje.reduccionCapital.cupon.confirmado.error");
		try {
			...
			reduccionCapital.getCupon().setEstadoCupon(reduccionCapitalDao.getEstadoCupon(estadoCupon.getId()));

			Estado estado = new Estado();
			estado.setIdestado(Constants.ANEXO_MODIF_ESTADO_ERROR.shortValueExact());
	...
	private Map<String, String> generarMsgConfirmacion(boolean indRevAdm, ReduccionCapital reduccionCapital, AcuseRecibo acuse)
			throws DAOException {

		String mensaje = "";
		String alerta = "";
		Map<String, String> mensajes = new HashMap<String, String>();
		final String MSG_CUPON_OK = getStrProperties("mensaje.reduccionCapital.cupon.confirmado.ok");
		final String MSG_CUPON_RECHAZADO = getStrProperties("mensaje.reduccionCapital.cupon.confirmado.rechazo");
		final String MSG_CUPON_TRAMITE = getStrProperties("mensaje.reduccionCapital.cupon.confirmado.tramite");
		final String MSG_CUPON_OK_REVADM = getStrProperties("mensaje.reduccionCapital.cupon.confirmado.revAdm");
		...
		if (acuse != null && acuse.getDocumentoArray() != null) {
			...
			if (estadoAcuse == Constants.ACUSE_RECIBO_ESTADO_CORRECTO) {
				...
				EstadoCuponRC estadoCuponRC = new EstadoCuponRC();
				estadoCuponRC.setId(indRevAdm ? Constants.AM_CUPON_ESTADO_CONFIRMADO_TRAMITE
						: Constants.AM_CUPON_ESTADO_CONFIRMADO_APLICADO);
				reduccionCapital.getCupon().setEstadoCupon(reduccionCapitalDao.getEstadoCupon(estadoCuponRC.getId()));

				Estado estado = new Estado(Constants.ANEXO_MODIF_ESTADO_CORRECTO.shortValueExact());
				...
				Estado estado = new Estado(Constants.ANEXO_MODIF_ESTADO_ERROR.shortValueExact());
				estado.setDescestado(Constants.ANEXO_MODIF_ESTADO_ERROR_DESC);
				reduccionCapital.setEstado(estado);
	...
	@Override
	public Map<String, Object> getAcuseConfirmacion(long idRC) {
		Map<String, Object> mapa = new HashMap<String, Object>();

		Clob acuse = null;
		try {
			acuse = reduccionCapitalDao.getAcuseConfirmacion(idRC);
		} catch (Exception e) {
			logger.error("Ha ocurrido un error al obtener el ultimo acuse de recibo del anexo " + idRC, e);
		}
	...
	private String generarMsgConfDatosAsociados(DatosAsociados da) {
		...
		if (estado != null) {
			if ("A".equals(estado))
				mensaje = RC_REF + " ha sido aceptado";
			else if ("E".equals(estado))
				mensaje = RC_REF + " ha sido aceptado pendiente de revision administrativa";
		}
		return mensaje;
	}
...
//	private static final String ANX_REF = "El Anexo de Modificacion con referencia ";
	private static final String ERR_ACT_XML =  "Error al actualizar el xml de ReduccionCapital";
//	
...
//
//		logger.debug("Iban2AsegModificado adicional obtenido de BBDD: " + anexo.getIban2AsegModificado());
...
//	 * @param am
//	 */
	private void actualizarXMLRedCap(XmlObject p, ReduccionCapital redCap) {
		redCap.setXml(Hibernate.createClob(p.toString()));
		try {
			reduccionCapitalDao.saveReduccionCapital(redCap);
		} catch (Exception e2) {
			logger.error(ERR_ACT_XML, e2);
		}
	}
...
//	}
//
	private void registrarComunicacionSW(final String codUsuario, ReduccionCapital reduccionCapital,String xmlPoliza, String XMLRespuesta) {
		try {
			AnexoRCSWValidacion anexoRed = new AnexoRCSWValidacion();
			anexoRed.setFecha(new Date());
			anexoRed.setIdCupon(reduccionCapital.getCupon().getId());
			anexoRed.setCodusuario(codUsuario);
			anexoRed.setReduccionCapital(reduccionCapital);
			anexoRed.setXmlEnvio(Hibernate.createClob(xmlPoliza));
			anexoRed.setXmlRespuesta(Hibernate.createClob(XMLRespuesta));
			reduccionCapitalDao.saveOrUpdate(anexoRed);
		} catch (DAOException e) {
...
//	}
	...
	public IEnviosSWConfirmacionDao getEnviosSWConfirmacionDao() {
		return enviosSWConfirmacionDao;
	}
	...
	public IHistoricoEstadosManager getHistoricoEstadosManager() {
		return historicoEstadosManager;
	}
