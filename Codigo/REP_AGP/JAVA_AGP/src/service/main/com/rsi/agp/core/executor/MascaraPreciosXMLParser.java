...
import com.rsi.agp.dao.tables.cpl.MascaraPrecioId;

public class MascaraPreciosXMLParser extends GenericXMLParser {
	...
	public static void main(String[] args){
		...
		if (args.length != 4) {
			System.out.println("Usage: java "
					+ MascaraPreciosXMLParser.class.getName()
					+ " <XML that needs to be transformed>" + " <Output file>" + " <LineaseguroId>" + " <dateFormat>");
			System.exit(1);
		}
		try {
			MascaraPreciosXMLParser parser = new MascaraPreciosXMLParser();
			parser.setTagPrincipal(GenericXMLParser.TAG_RG);
			parser.procesarFichero(args[0], args[1], new Long(args[2]), args[3]);
		} catch (FileNotFoundException e) {
			System.out.println("Error al buscar el fichero el fichero de máscara de precios " + e.getMessage());
			System.exit(2);
		} catch (SAXException e) {
			System.out.println("Error al crear el XMLReader " + e.getMessage());
			System.exit(3);
		} catch (IOException e) {
			System.out.println("Error de entrada/salida al parsear el fichero de máscara de precios " + e.getMessage());
			System.exit(4);
	...
	protected String generaInsert(Object reg, String dateFormat) {
		MascaraPrecio registro = (MascaraPrecio)reg;
		String sql = "";
	...
	protected Object generaRegistro(Object actual, String tag, XMLStreamReader parser, int id, Long lineaseguroid) {
		MascaraPrecio registro;
		if (actual == null){
			registro = new MascaraPrecio();
		}
		else{
			registro = (MascaraPrecio) actual;
		}
		
		if (this.getTagPrincipal().equals(tag)){
			MascaraPrecioId idMasc = new MascaraPrecioId();
