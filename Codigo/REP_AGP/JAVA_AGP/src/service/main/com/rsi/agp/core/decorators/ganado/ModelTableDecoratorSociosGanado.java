package com.rsi.agp.core.decorators.ganado;
...
public class ModelTableDecoratorSociosGanado extends TableDecorator 
{
	...
	public String getSocioNombre ()
	{
		...
		if(so.getTipoidentificacion().equalsIgnoreCase("CIF") && so.getRazonsocial()!=null){
			cadena = so.getRazonsocial().toString();
			   ...
			   cadena = so.getNombre().toString() + " " +  so.getApellido1().toString();
		      }
	...
	public String getSubvDeclaradas ()
	{
		...
		//Obtengo el identificador de la poliza para saber si el socio tiene subvenciones para la misma.
		...
		if (p != null && so.getSubvencionSocioGanados()!= null && so.getSubvencionSocioGanados().size() > 0)
		{
			for (SubvencionSocioGanado ss: so.getSubvencionSocioGanados()){
