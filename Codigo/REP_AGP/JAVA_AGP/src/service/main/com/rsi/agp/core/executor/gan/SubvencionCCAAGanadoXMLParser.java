...
import com.rsi.agp.dao.tables.cpl.gan.SubvencionCCAAGanadoId;
import com.rsi.agp.dao.tables.cpl.gan.TiposAnimalGanado;
import com.rsi.agp.dao.tables.cpl.gan.TiposAnimalGanadoId;

/**
 * Parser tabla 0436: Subvenciones CC.AA.
 * @author U029823
 *
 */
public class SubvencionCCAAGanadoXMLParser extends GenericXMLParser {
...
//		args[3] = GenericXMLParser.formatoOrigen;
		...
		if (args.length != 4) {
			System.out.println("Usage: java " + SubvencionCCAAGanadoXMLParser.class.getName()
					+ " <XML that needs to be transformed>" + " <Output file>" + " <LineaseguroId>" + " <dateFormat>");
			System.exit(1);
		}
		try {
			SubvencionCCAAGanadoXMLParser parser = new SubvencionCCAAGanadoXMLParser();
			parser.setTagPrincipal(GenericXMLParser.TAG_RG);
			parser.procesarFichero(args[0], args[1], new Long(args[2]), args[3]);
		} catch (FileNotFoundException e) {
			System.out.println("Error al buscar el fichero el fichero de subvenciones CC.AA. " + e.getMessage());
			e.printStackTrace();
			System.exit(2);
			...
			System.out.println("Error de entrada/salida al parsear el fichero de subvenciones CC.AA. " + e.getMessage());
			e.printStackTrace();
			System.exit(4);
	...
	protected String generaInsert(Object reg, String dateFormat) {
		...
		SubvencionCCAAGanado registro = (SubvencionCCAAGanado)reg;
		String sql = "";
		sql += StringUtils.nullToString(registro.getId().getId()) + ";";
		sql += registro.getId().getLineaseguroid() + ";";
		sql += registro.getOrganismo().getCodorganismo() + ";";
		...
		sql += StringUtils.nullToString(registro.getTipoSubvencionCCAA().getCodtiposubvccaa()) + ";";
		
		if(registro.getTasaComercialBase()!=null){
	...
	protected Object generaRegistro(Object actual, String tag, XMLStreamReader parser, int id, Long lineaseguroid) {
		...
		SubvencionCCAAGanado registro;
		SubvencionCCAAGanadoId idActual;
		if (actual == null){
			registro = new SubvencionCCAAGanado();
			idActual = new SubvencionCCAAGanadoId();
			...
			registro = (SubvencionCCAAGanado) actual;
		...
		if (this.getTagPrincipal().equals(tag)){
			...
			if (!StringUtils.nullToString(parser.getAttributeValue(null, "codOrg")).equals("")){
				...
				Character codOrganismo = new Character(StringUtils.nullToString(parser.getAttributeValue(null, "codOrg").trim()).charAt(0));
				organismo.setCodorganismo(codOrganismo);
				registro.setOrganismo(organismo);
			}
			...
			if (!StringUtils.nullToString(parser.getAttributeValue(null, "tipSubv")).equals("")){
				TipoSubvencionCCAA tipoSubvencion = new TipoSubvencionCCAA();
				BigDecimal codTipoSubvencion = new BigDecimal(StringUtils.nullToString(parser.getAttributeValue(null, "tipSubv")));
				tipoSubvencion.setCodtiposubvccaa(codTipoSubvencion);
				registro.setTipoSubvencionCCAA(tipoSubvencion);
			}
			
			//tasaComBase
			if (!StringUtils.nullToString(parser.getAttributeValue(null, "tasaComBase")).equals("")){
				registro.setTasaComercialBase(new BigDecimal (StringUtils.nullToString(parser.getAttributeValue(null, "tasaComBase"))));
			}
