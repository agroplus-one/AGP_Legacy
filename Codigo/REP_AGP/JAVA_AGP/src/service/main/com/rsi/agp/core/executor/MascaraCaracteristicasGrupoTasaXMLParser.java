...
import com.rsi.agp.dao.tables.cpl.MascaraGrupoTasasId;

public class MascaraCaracteristicasGrupoTasaXMLParser extends GenericXMLParser {
	...
	public static void main(String[] args){
		...
		if (args.length != 4) {
			System.out.println("Usage: java "
					+ MascaraCaracteristicasGrupoTasaXMLParser.class.getName()
					+ " <XML that needs to be transformed>" + " <Output file>" + " <LineaseguroId>" + " <dateFormat>");
			System.exit(1);
		}
		try {
			MascaraCaracteristicasGrupoTasaXMLParser parser = new MascaraCaracteristicasGrupoTasaXMLParser();
			parser.setTagPrincipal(GenericXMLParser.TAG_RG);
			parser.procesarFichero(args[0], args[1], new Long(args[2]), args[3]);
		} catch (FileNotFoundException e) {
			System.out.println("Error al buscar el fichero el fichero de máscara de fechas de características del grupo de tasas " + e.getMessage());
			e.printStackTrace();
			System.exit(2);
			...
			System.out.println("Error de entrada/salida al parsear el fichero de máscara de fechas de características del grupo de tasas " + e.getMessage());
			e.printStackTrace();
			System.exit(4);
	...
	protected String generaInsert(Object reg, String dateFormat) {
		MascaraGrupoTasas registro = (MascaraGrupoTasas)reg;
		String sql = "";
	...
	protected Object generaRegistro(Object actual, String tag, XMLStreamReader parser, int id, Long lineaseguroid) {
		MascaraGrupoTasas registro;
		if (actual == null){
			registro = new MascaraGrupoTasas();
		}
		else{
			registro = (MascaraGrupoTasas) actual;
		}
		
		if (this.getTagPrincipal().equals(tag)){
			MascaraGrupoTasasId idMasc = new MascaraGrupoTasasId();
