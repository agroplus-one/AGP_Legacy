...
import com.rsi.agp.core.jmesa.service.IFicheroDeudaAplazadaApliService;
...
public class FicheroDeudaAplazadaApliService implements IFicheroDeudaAplazadaApliService{
	...
	public static final String CAMPO_ID_EMITIDO		 ="reciboDeudaAplazada.id";
	...
	public static final String CAMPO_APELLIDO2       ="apellido2";
	 ...
	 public int getFicheroDeudaAplazadaApliCountWithFilter(DeudaAplazadaApliFilter filter,
			 DeudaAplazadaAplicacion deudaAplazadaApli) {
		 return deudaAplazadaDao.getFicheroDeudaAplazadaApliCountWithFilter(filter,
				 deudaAplazadaApli);
	}
	...
	public Collection<DeudaAplazadaAplicacion> getFicheroDeudaAplazadaApliWithFilterAndSort(
			...
			int rowEnd, DeudaAplazadaAplicacion deudaAplazadaApli) throws BusinessException {

		return deudaAplazadaDao.getFicheroDeudaAplazadaApliWithFilterAndSort(filter, sort,
				rowStart, rowEnd, deudaAplazadaApli);
	}
	
	
	@Override
	public String getTablaDeudaAplazadaApli(HttpServletRequest request,
		...
		TableFacade tableFacade = crearTableFacade(request, response,
				deudaAplazadaApli, origenLlamada);

		setDataAndLimitVariables(tableFacade, deudaAplazadaApli);
	...
	private void cargarFiltrosBusqueda(DeudaAplazadaAplicacion deudaAplazadaApli,
			TableFacade tableFacade) {
		
			tableFacade.getLimit().getFilterSet().addFilter(
					"deudaAplazada.id",
					deudaAplazadaApli.toString());

	}
	
	private void setDataAndLimitVariables(TableFacade tableFacade,
			DeudaAplazadaAplicacion emitidoApli) {

		// Obtiene el Filter para la búsqueda de ficheros
		Limit limit = tableFacade.getLimit();
		DeudaAplazadaApliFilter emitidosApliFilter = getDeudaAplazadaApliFilter(limit);

		// Obtiene el número de filas que cumplen el filtro
		int totalRows = getFicheroDeudaAplazadaApliCountWithFilter(emitidosApliFilter,
				emitidoApli);
		logger
		...
		DeudaAplazadaApliSort emitidosApliSort = getDeudaAplazadaApliSort(limit);
		int rowStart = limit.getRowSelect().getRowStart();
		int rowEnd = limit.getRowSelect().getRowEnd();

		// Obtiene los registros que cumplen el filtro
		Collection<DeudaAplazadaAplicacion> items = new ArrayList<DeudaAplazadaAplicacion>();
		
		try {
			items = getFicheroDeudaAplazadaApliWithFilterAndSort(emitidosApliFilter,
					emitidosApliSort, rowStart, rowEnd, emitidoApli);
			logger.debug("********** Items de la lista   = "
					+ items.size() + " **********");
			for (Iterator<DeudaAplazadaAplicacion> iterator = items.iterator(); iterator.hasNext();) {
				DeudaAplazadaAplicacion c = (DeudaAplazadaAplicacion) iterator.next();
				/*
	 ...
	 * @return
	 */
	private DeudaAplazadaApliFilter getDeudaAplazadaApliFilter(Limit limit) {
		DeudaAplazadaApliFilter emitidosApliFilter = new DeudaAplazadaApliFilter();
	 ...
	 * @return
	 */
	private DeudaAplazadaApliSort getDeudaAplazadaApliSort(Limit limit) {
		DeudaAplazadaApliSort emitidosApliSort = new DeudaAplazadaApliSort();
	...
	private CellEditor getCellEditorAcciones() {
		return new CellEditor() {
			public Object getValue(Object item, String property, int rowcount) {
				...
				String idDeudaAplazada = new BasicCellEditor().getValue(item,
						CAMPO_ID_EMITIDO, rowcount).toString();
				String idDeudaAplazadaApli = new BasicCellEditor().getValue(item,
						CAMPO_ID_EMITIDO_APLI, rowcount).toString();
				HtmlBuilder html = new HtmlBuilder();
				
				// botón consulta
				html.a().href().quote().append(
						"javascript:detalle('" + idDeudaAplazada + "','"+idDeudaAplazadaApli+"');").quote()
						.close();
				html
				...
				html.a().href().quote().append(
						"javascript:guardaIdDeudaAplazadaApli('"+idDeudaAplazadaApli+"');lupas.muestraTabla('DeudaAplazadaApliSubvCCAA','principio', '', '');").quote()
						.close();
				html
	...
	@Override
	public DeudaAplazadaAplicacion getDatosDeudaAplazada(Long idDeudaAplazada) throws BusinessException {
		try{
			return (DeudaAplazadaAplicacion) deudaAplazadaDao.getObject(DeudaAplazadaAplicacion.class, idDeudaAplazada);
	...
	public void setDeudaAplazadaDao(IDeudaAplazadaDao deudaAplazadaDao) {
		this.deudaAplazadaDao = deudaAplazadaDao;
	}
}