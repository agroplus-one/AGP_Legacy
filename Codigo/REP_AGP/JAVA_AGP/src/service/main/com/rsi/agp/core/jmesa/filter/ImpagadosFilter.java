...
public class ImpagadosFilter implements CriteriaCommand{
	...
	private void buildCriteria(Criteria criteria, String property, Object value) {
        if (value != null) {
			...
			try {
				if (property.equals("recibo")){
	    			criteria.add(Restrictions.eq("recibo", new BigDecimal(value.toString())));
	    		}
	    		...
	    		}else if (property.equals("fichero.id")){
	    			criteria.add(Restrictions.eq("fichero.id", new Long(value.toString())));
	    		}
	    		
	        	
	        }catch (Exception e) {
				logger.error("ImpagadosFilter - "+e.getMessage());
			}     	
        }
    }
